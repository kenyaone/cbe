...
class LinkGradeFourContentSeeder extends Seeder
{
    public function run()
    {
        $storagePath = '/home/tele/cbe-platform/storage/app/media/Grade Four Complete';

        if (!is_dir($storagePath)) {
            $this->command->error("Grade Four content not found at {$storagePath}");
            return;
        }
        ...
        foreach ($subjectMap as $folder => $subjectName) {
            $defaultSubStrands[$folder] = SubStrand::whereHas('strand', function($q) {
                $q->whereHas('learningArea', function($qa) {
                    $qa->where('grade_level', 'Grade Four');
                });
        ...
        $this->command->info("✓ Created: $created Grade Four content files");

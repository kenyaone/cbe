...
class SimplifyGradeFourStructure extends Seeder
{
    public function run()
    {
        $this->command->info('=== SIMPLIFYING GRADE FOUR ===');
        $this->command->info('Converting: Subject → Strand → SubStrand → Content');
        $this->command->info('To:         Subject → Lesson (Video/HTML directly)');
        $this->command->info('');

        $subjects = LearningArea::where('grade_level', 'Grade Four')->get();
        ...
        $this->command->info('');
        $this->command->info('✅ Grade Four simplified successfully!');
    }
}

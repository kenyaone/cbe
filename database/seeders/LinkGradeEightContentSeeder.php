...
class LinkGradeEightContentSeeder extends Seeder
{
    public function run()
    {
        $storagePath = '/home/tele/cbe-platform/storage/app/media/Grade Eight Complete';

        if (!is_dir($storagePath)) {
            $this->command->error("Grade Eight content not found at {$storagePath}");
            return;
        }
        ...
        foreach ($subjectMap as $folder => $subjectName) {
            $defaultSubStrands[$folder] = SubStrand::whereHas('strand', function($q) {
                $q->whereHas('learningArea', function($qa) {
                    $qa->where('grade_level', 'Grade Eight');
                });
        ...
        $this->command->info("✓ Created: $created Grade Eight content files");

...
class LinkGradeThreeContentSeeder extends Seeder
{
    public function run()
    {
        $storagePath = '/home/tele/cbe-platform/storage/app/media/Grade Three Complete';

        if (!is_dir($storagePath)) {
            $this->command->error("Grade Three content not found at {$storagePath}");
            return;
        }
        ...
        foreach ($subjectMap as $folder => $subjectName) {
            $defaultSubStrands[$folder] = SubStrand::whereHas('strand', function($q) {
                $q->whereHas('learningArea', function($qa) {
                    $qa->where('grade_level', 'Grade Three');
                });
        ...
        foreach ($files as $filePath => $fileType) {
            ...
            foreach (array_keys($defaultSubStrands) as $folder) {
                if (stripos($relativePath, $folder) !== false || stripos($filename, $folder) !== false) {
        ...
        $this->command->info("✓ Created: $created Grade Three content files");

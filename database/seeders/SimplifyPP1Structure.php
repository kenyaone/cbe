...
class SimplifyPP1Structure extends Seeder
{
    public function run()
    {
        $this->command->info('=== SIMPLIFYING PP1 ===');
        $this->command->info('Converting: Subject → Strand → SubStrand → Content');
        $this->command->info('To:         Subject → Lesson (Video/HTML directly)');
        $this->command->info('');

        $subjects = LearningArea::where('grade_level', 'PP1')->get();
        ...
        $this->command->info('');
        $this->command->info('✅ PP1 simplified successfully!');
    }
}

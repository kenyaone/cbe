...
class RemapPP2EnglishVideos extends Seeder
{
    public function run()
    {
        $this->command->info('=== REMAPPING PP2 ENGLISH VIDEOS ===');

        // Get Language Activities subject
        $langSubject = LearningArea::where('grade_level', 'PP2')
        ...
        $englishPatterns = [
            'abc', 'letter', 'vowel', 'consonant', 'greet', 'language', 'word'
        ];

        // Find English videos currently in Math
        $mathSubject = LearningArea::where('grade_level', 'PP2')

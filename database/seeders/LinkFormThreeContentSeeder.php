...
class LinkFormThreeContentSeeder extends Seeder
{
    public function run()
    {
        $basePath = '/media/tele/CBE/Form three';

        if (!is_dir($basePath)) {
            $this->command->error("Form Three content directory not found: $basePath");
            return;
        }
        ...
        foreach ($iterator as $file) {
            ...
            // Intelligently assign to Form Three sub-strand
            $subStrand = $this->assignToSubStrand($filePath, $title);

            if (!$subStrand) {
                // Fallback: assign to first Form Three subject's first sub-strand
                $subStrand = SubStrand::whereHas('strand.learningArea', fn($q) =>
                    $q->where('grade_level', 'Form Three')
                )->first();
        ...
        $this->command->info("Linked $count Form Three content files");
    }

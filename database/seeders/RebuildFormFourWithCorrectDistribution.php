...
class RebuildFormFourWithCorrectDistribution extends Seeder
{
    public function run()
    {
        $this->command->info('=== REBUILDING FORM FOUR WITH CORRECT DISTRIBUTION ===');
        $this->command->info('Videos and 2 PDFs in Lessons\n');

        // Fix curriculum
        LearningArea::where('grade_level', 'Form Four')->update(['curriculum_type_id' => 1]);

        $basePath = '/home/tele/cbe-platform/storage/app/media/form-four';
        $videoType = ContentType::firstOrCreate(['name' => 'Video']);
        $pdfType = ContentType::firstOrCreate(['name' => 'PDF']);

        // Clear existing content
        $subjects = LearningArea::where('grade_level', 'Form Four')->get();
        ...
        $config = [
            ...
            'Chemistry' => [
                ...
                'pdf_patterns' => ['KCSE FORM 4 CHEMISTRY'],
            ],
            'Physics' => [
            ...
            'Christian Religious Education' => [
                'video_folder' => null,
                'pdf_patterns' => ['CRE FORM 4'],
            ],
        ];

        foreach ($config as $subjectName => $settings) {
            $subject = LearningArea::where('grade_level', 'Form Four')
            ...
            // Link PDFs from Form Four folder (only if patterns defined)
            if (count($settings['pdf_patterns']) > 0 && $lessonsStrand) {
                $folderPath = "$basePath/Form Four";
        ...
        $this->command->info('');
        $this->command->info('✅ Form Four rebuilt with correct distribution!');
    }

...
class LinkGradeTwoContentSeeder extends Seeder
{
    public function run()
    {
        $storagePath = '/home/tele/cbe-platform/storage/app/media/Grade Two Complete';

        if (!is_dir($storagePath)) {
            $this->command->error("Grade Two content not found at {$storagePath}");
            return;
        }
        ...
        foreach ($subjectMap as $folder => $subjectName) {
            $defaultSubStrands[$folder] = SubStrand::whereHas('strand', function($q) {
                $q->whereHas('learningArea', function($qa) {
                    $qa->where('grade_level', 'Grade Two');
                });
        ...
        $this->command->info("✓ Created: $created Grade Two content files");

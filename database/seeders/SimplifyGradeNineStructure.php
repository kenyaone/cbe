...
class SimplifyGradeNineStructure extends Seeder
{
    public function run()
    {
        $this->command->info('=== SIMPLIFYING GRADE NINE ===');
        ...
        foreach ($subjects as $subject) {
            ...
            // Create "Lessons" strand for videos/HTML (use unique timestamp-based code if conflict)
            $code = $subject->code . 'LESS';
            $counter = 1;
            while (Strand::where('learning_area_id', $subject->id)->where('code', $code)->exists()) {
                $code = $subject->code . 'LESS' . $counter++;
            }
        ...
        $this->command->info('');
        $this->command->info('✅ Grade Nine simplified successfully!');
        $this->command->info('Ready to link unlinked videos to new Lessons strands.');
    }
}

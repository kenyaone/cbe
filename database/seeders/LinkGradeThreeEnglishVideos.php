...
class LinkGradeThreeEnglishVideos extends Seeder
{
    public function run()
    {
        $this->command->info('=== LINKING GRADE THREE ENGLISH VIDEOS ===');

        $basePath = '/home/tele/cbe-platform/storage/app/media/Grade Three Complete/English';

        if (!is_dir($basePath)) {
            $this->command->error("Grade Three English directory not found");
            return;
        }

        // Get English subject
        $englishSubject = LearningArea::where('grade_level', 'Grade Three')

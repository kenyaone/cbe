...
class SimplifyGradeTenStructure extends Seeder
{
    public function run()
    {
        $this->command->info('=== SIMPLIFYING GRADE TEN ===');
        $this->command->info('Converting: Subject → Strand → SubStrand → Content');
        $this->command->info('To:         Subject → Lesson (Video/HTML directly)');
        $this->command->info('');

        $subjects = LearningArea::where('grade_level', 'Grade Ten')->get();
        ...
        $this->command->info('');
        $this->command->info('✅ Grade Ten simplified successfully!');
    }
}

...
class LinkGradeSixScienceVideos extends Seeder
{
    public function run()
    {
        $this->command->info('=== LINKING GRADE SIX SCIENCE VIDEOS ===');

        $basePath = '/home/tele/cbe-platform/storage/app/media/Grade Six Complete/Videos Science';

        if (!is_dir($basePath)) {
            $this->command->error("Grade Six Science Videos directory not found");
            return;
        }

        // Get Science subject
        $scienceSubject = LearningArea::where('grade_level', 'Grade Six')
        ...
        $this->command->info('');
        $this->command->info("✅ Linked {$videoCount} Science videos");
    }
}

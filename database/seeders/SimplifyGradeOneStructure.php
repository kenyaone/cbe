...
class SimplifyGradeOneStructure extends Seeder
{
    public function run()
    {
        ...
        foreach ($subjects as $subject) {
            ...
            // Create SubStrand for each video/HTML file
            $lessonOrder = 0;
            foreach ($contentFiles as $contentFile) {
                if (!in_array($contentFile->contentType->name, ['Video', 'Interactive'])) {
                    continue; // Skip PDFs - keep separate
                }

                $lessonOrder++;

                // Create SubStrand (one per video/HTML)
                ...
                // Update content file to point to new SubStrand
                $contentFile->update([
            ...
            $this->command->line("  Created {$lessonOrder} lesson SubStrands from videos/HTML");

            // Handle PDFs in separate "Study Materials" strand if they exist
            ...
            if ($pdfFiles->count() > 0) {
                ...
                $this->command->line("  Created Study Materials section for {$pdfFiles->count()} PDFs");
            }
        }

        $this->command->info('');
        $this->command->info('✅ Grade One simplified successfully!');
        $this->command->info('Structure: Grade One → Subject → Video Lessons');
    }
}

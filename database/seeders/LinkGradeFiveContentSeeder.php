...
class LinkGradeFiveContentSeeder extends Seeder
{
    public function run()
    {
        $storagePath = '/home/tele/cbe-platform/storage/app/media/Grade Five Complete';

        if (!is_dir($storagePath)) {
            $this->command->error("Grade Five content not found at {$storagePath}");
            return;
        }
        ...
        foreach ($subjectMap as $folder => $subjectName) {
            $defaultSubStrands[$folder] = SubStrand::whereHas('strand', function($q) {
                $q->whereHas('learningArea', function($qa) {
                    $qa->where('grade_level', 'Grade Five');
                });
        ...
        $this->command->info("✓ Created: $created Grade Five content files");
